...
app.whenReady().then(() => {
  ...
  //   // ipcMain.send("sendToken",token);
  //   store.set('fcm_token', token);
  // });
  ipcMain.on("showNotification", (e, push) => {
    console.log('xxxx',push);
    var notify=new Notification({title:push.notification.title,body:push.notification.body});
    notify.show();
    notify.click = () => {
